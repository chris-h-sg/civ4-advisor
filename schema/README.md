...
- **`state.schema.json`** — JSON Schema (draft 2020-12) for the per-turn state file the mod writes (`state/current_turn.json`). This is the reference for field names, types, and visibility semantics; the field descriptions inside it are the documentation.
