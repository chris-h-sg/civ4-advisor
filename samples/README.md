...
- **`baseline-early-game/`** — turns 0–43, the default-settings reference run. Portugal (João), Emperor, Fractal, standard/temperate/medium, 7 civs, no game options set. Two cities: Lisbon founded t1 at (75,36), reaching pop 2 at t39; Oporto founded t36 at (78,37). Three rivals met — Greece t15, Rome t24, Ottoman t27 — all `ATTITUDE_CAUTIOUS`, never at war. 29 → 266 tiles revealed, 108 still visible at t43. Deliberately ordinary: it is the control the others are varied against.
  ...
  **Fourth provenance note — `units[].damage` in this run predates the heal-prediction fix (see `CLAUDE.md`'s `units[].damage` bullet) and was left unmigrated.** Not a clean mechanical transform like the y-axis flip: reconstructing the predicted values from position data alone correctly explained 10 of the 14 damaged-unit-turns in this run (both units, flat neutral-territory healing, no promotions or nearby units involved) but left 4 unexplained — no visible cause in the exported data, most plausibly a same-turn move/heal interaction this reconstruction doesn't model. Left as-is rather than guessing at the remaining 4. Only two units are ever damaged in this run (`24576`, `16385`, turns 11–20 and 16–19), so the affected surface is small and doesn't touch anything else the run is curated for.
...
- **Varied setups** — the `game` section's climate/sea-level/barbarian options change early-game advice more than almost anything else in the file, so runs that differ there are worth more than repeats of the same conditions. **Which specific gaps are worth targeting next is in [`ROADMAP.md`](../ROADMAP.md)**, which is where that list is maintained.
