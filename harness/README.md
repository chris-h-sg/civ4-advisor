...
**"What can I build right now" was asked for and refused.** A tech chooser was requested by one trial and the same argument extends to buildings, but the answer is **per city**, not per empire: `bWater` gates on coastal, `bRiver` on river, and prereq buildings are per city, so a single empire-wide list would be wrong for at least one city most of the time. It is also one step from the query language this folder refuses. The lookups being cheap to call repeatedly serves the underlying need — comparison — without inventing a list that lies.
...
**Findings recorded rather than built**, all presentation and all evidenced, so they are not re-derived from scratch later:

- **Per-city buildability, and "how fast can I field X"** (2 of 4 trials). One made five `rules.py unit` calls guessing type names to discover four were unavailable, and said it was *relying on memory to know which units to even ask about* — the failure the guide's rule 5 names. Both requests are scoped to a **named city**, which answers the objection that killed the empire-wide chooser: `bWater`, `bRiver` and prereq buildings vary per city, but `--city Lisbon` does not have to generalise. Wanted alongside it: time-to-first-unit crossing tech *and* hammers, which currently stops dead at the tech.
- **The tech↔map resource join, in the positive direction** (2 of 4). `rules.py tech TECH_MASONRY` prints `resources usable BONUS_STONE` while the player already has stone inside their borders — "a decisive fact printed nowhere". The unit view now does this; the tech view does not.
- **Turns-to-complete on what a city is currently building** (2 of 4). The arithmetic already exists in `rules.py building`; `cities[].producing` plus `production`/`productionNeeded` is not joined to it anywhere.
- **The research-switch rule** (1 of 4, but a rule-5 violation with no fallback). Deciding whether to abandon a tech at 4/124 required knowing Civ IV banks partial research per tech, which the agent asserted from background knowledge and flagged as exactly what the guide forbids.
- **A stagnating-city flag.** A city frozen at size 1 on +0 food was one trial's most actionable finding, recovered with an ad-hoc script. `--view yields` prints the total and does not say the assignment cannot grow.
- **Multi-site `--around` comparison** (asked in both rounds; one trial ran it ten times and diffed by eye). Tabulating the same facts across sites is not ranking them.
- **`intel`'s empty-city line is buried** — now **7 of 10 trials** across both rounds, one saying it nearly scrolled past the most important line in the output.
- **The wrap-seam header** (`x 64-4`) reads as a backwards range — **5 of 10**.

**Four findings landed on the other tools rather than this one**, recorded here with their evidence so they are not re-derived: a **threat/approach report** (asked for by four of six; two agents independently wrote their own BFS over `map.tiles`, one calling it the load-bearing step of its answer — `intel` says to check `--view military` and `military` cannot answer it either); **city production in `intel`'s garrison block** (four of six read raw JSON for it after `intel` told them a city was empty; one found that Lisbon switched *off* a Warrior on t40 while undefended, the sharpest fact in the run, visible in no view); **the empty-city line buried at the bottom of `intel`** (four of six, one saying it nearly scrolled past the most important line in the output); and **the wrap-seam range** `x 66-2` reading as a typo (three of six).
...
3. ~~**Rules lookup — narrow.**~~ **Done** — `rules.py` above, `unit`/`tech`/`handicap`. Three departures from the sketch, all from measuring rather than predicting. **Handicap was added despite being a forward lookup**, on the roadmap's own reasoning rather than its wording: "the agent greps forward lookups fine" is falsified for these fields by two trials that changed conclusions on them. **The state file became mandatory** once the cost multipliers were measured — game speed × world size × handicap put a base XML cost 1.0–4.5× off, so an unpriced lookup is not a cheaper answer but a wrong one. And **depth turned out not to need a limit**: the tree is 92 techs wide and shallow, largest full closure 14, so the full walk is always affordable and a truncated one would be the exact failure the tool exists to prevent.

   The tool's premise got validated three times during its own construction: a hand-grep, a remembered combat modifier and a worked example each produced a confident wrong answer that the parsed XML caught (details in the design notes).

4. **A city threat/approach report — the best-evidenced gap in the folder.** Four of six `rules.py` trials asked for it and **two independently wrote their own BFS** over `map.tiles` to get it, which is this repo's stated signal for a missing tool. The specific failure: `intel` correctly refuses to judge whether an empty city is in danger and points at `--view military`; `military` renders a symbol grid and cannot answer it either, so the agent eyeballs a corridor off the grid — the derivation the guide says is unreliable. What is wanted is presentation, not a verdict: per city, the distance to the nearest unowned land tile, how many reachable unowned land tiles are within N walk, and whether there is a land route at all. The walk-distance machinery already exists in `run_history.py` and `State.land_distance`. The line to hold is the one `intel` already holds — report the approach, never call it safe or unsafe.

5. **Per-city production over the run.** Four of six trials read raw JSON for what a city was building after `intel` told them it was empty; `timeline` reports techs, cities, units, contacts, sightings, tiles, territory and resources, but not what any city is *producing* or when that changed. One trial found that Lisbon started a Warrior on t38 while both cities were empty and switched **back** to a Worker on t40 — the sharpest fact in the whole run, visible in no view, recovered with a throwaway loop over five files. Two obvious halves: a `producing` change line in `timeline`, and current build plus turns-to-complete on `intel`'s garrison block.

6. **Per-city buildability — `rules.py <thing> <state> --city NAME`.** Asked by two of four second-round trials from opposite directions: one wanted to confirm a city could build a Barracks, one wanted to know which military units were available at all and burned five calls guessing type names to find out. Scoped to a **named city**, which is what makes it viable — the empire-wide "what can I build" list was refused because `bWater`, `bRiver` and prereq buildings vary per city, and naming the city removes that objection entirely. Increment ⑤ exported exactly these inputs (`cities[].buildings`, `cities[].bonuses`, `cities[].coastal`) for a derivation nothing performs. It presents blockers; it must not rank options.
